...
shinyUI(fluidPage(
    # Application title
    titlePanel(em(strong("Word Prediction", style = "color:#00008B; size=4"))),
            ...
            tabPanel("App information", 
                     fluidRow( 
                         column(8, br(),
                                ...
                                p("- Word Prediction employs a  Markov process and is based on a modified 'Stupid Backoff' algorithm. Using all input N-grams (4-grams - 1-grams) it calculates the probability for all the words following that N-gram (maximum likelihood estimator or MLE). These probabilities are summed across all N-gram collections (1 to k) to obtain a weight for that word (W)."),
                                br(),
